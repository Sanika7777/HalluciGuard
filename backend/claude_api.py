...
def _call_claude(system: str, user: str, max_tokens: int = 1000, retries: int = 2) -> str:
    """
    ...
    for attempt in range(retries + 1):
        try:
            ...
            log.warning(f"Claude rate limited (attempt {attempt+1})", extra={"error": str(e)})
            last_exc = e
            if attempt < retries:
                time.sleep(2 ** attempt)    # 1s, 2s backoff

        except APITimeoutError as e:
            log.warning(f"Claude timeout (attempt {attempt+1})", extra={"error": str(e)})
            last_exc = e
            if attempt < retries:
                time.sleep(1)

        except APIConnectionError as e:
            log.warning(f"Claude connection error (attempt {attempt+1})", extra={"error": str(e)})
            last_exc = e
            if attempt < retries:
                time.sleep(2)
...
def analyze_prompt_context(prompt: str, ml_result: dict, llm_target: str = "gpt4") -> dict:
    """
...
- Be specific, not generic"""
    ...
    try:
        ...
        required = {"missing_context", "why_risky", "what_to_add", "llm_specific_warning"}
        if not required.issubset(result.keys()):
            raise ValueError(f"Claude response missing keys: {required - result.keys()}")

        duration_ms = round((time.perf_counter() - t0) * 1000, 2)
        log.info("Claude context analysis complete", extra={
            "model": CLAUDE_MODEL,
        ...
        log.warning("Claude returned invalid JSON — using ML fallback", extra={"error": str(e)})
        return _fallback_context(ml_result)
    except Exception as e:
...
- Do NOT change the core intent of the original prompt"""
    ...
    try:
        ...
        duration_ms = round((time.perf_counter() - t0) * 1000, 2)
        log.info("Claude prompt engineering complete", extra={
            "model": CLAUDE_MODEL,
            "llm_target": llm_target,
            "duration_ms": duration_ms,
        ...
        log.warning("Claude returned invalid JSON for engineering — using fallback", extra={"error": str(e)})
        return _fallback_engineer(prompt, llm_target)
    except Exception as e:
...
def _fallback_context(ml_result: dict) -> dict:
    ...
    return {
        "missing_context": ml_result.get("missing_context", [
            "Domain or field of application",
            "Time period or date reference",
            "Specific constraints or requirements",
        ]),
        "why_risky": ml_result.get("why_risky", [
        ...
        "what_to_add": ml_result.get("what_to_add", [
            "Add domain context",
            "Specify time period",
